...
export default function EWalletPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const promoRes = await fetch("https://api-v2.bajiraj.cloud/promos");
      const promoData = await promoRes.json();

      const payRes = await fetch("https://api-v2.bajiraj.cloud/payment-gateways/");
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      await fetch("https://api-v2.bajiraj.cloud/deposit", {
        method: "POST",
    ...
    const fetchPhones = async () => {
      try {
        const res = await fetch(
          `https://api-v2.bajiraj.cloud/users/phones/${currentUser.id}`,
        );

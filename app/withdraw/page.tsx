...
const [delay, setDelay] = useState(10)
  ...
  useEffect(() => {
    ...
    const loadPhones = async () => {
      if (!u) return;
      try {
        const res = await fetch(`https://api-v2.bajiraj.cloud/users/phones/${u.id}`);
    ...
    const loadPaymentOptions = async () => {
      try {
        const res = await fetch("https://api-v2.bajiraj.cloud/payment-gateways/");
      ...
      if (!u) return;
      try {
        const res = await fetch(`https://api-v2.bajiraj.cloud/payment-gateways/widthraw`);

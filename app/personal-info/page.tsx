...
export default function ProfilePage() {
  ...
  const fetchPhones = async (userId: number) => {
    const res = await fetch(
      `https://api-v2.bajiraj.cloud/users/phones/${userId}`
    );
...
const backToHome = () =>{
  ...
  const addPhone = async () => {
    ...
    await fetch("https://api-v2.bajiraj.cloud/users/phone", {
      method: "POST",
...
if(otp == wpotp){
  ...
  const verifyPhone = async (phone: string) => {
    if (!user) return;
    await fetch("https://api-v2.bajiraj.cloud/users/phone/verify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },

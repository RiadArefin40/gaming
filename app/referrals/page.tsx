...
const ReferralPage = () => {
  ...
  useEffect(() => {
    const fetchReferrals = async () => {
      try {
        const res = await fetch(`https://api-v2.bajiraj.cloud/users/${myreferral}/referrals`);
...
const ReferralList = ({
  ...
  const handleClaim = async (bonusId: number, userId: number) => {
    try {
      const res = await fetch(`https://api-v2.bajiraj.cloud/users/${bonusId}/claim`, { method: "POST" });
